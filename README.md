...
Home import URL/zip is `POST /api/adopt` (`ADOPT_URL`), not `/api/assemble`. After Import, dest zip is `POST /api/adopt/zip` `{dest}` (the adopt tree, not etalon). Dest cloud/user push YAML may carry `adoptDest` and unzip that zip (`ADOPT_URL`), not `ASSEMBLE_URL`. Local compose default `http://host.docker.internal:3033` → `ensure.py adopt`. Empty URL → 503. Never a PAT.
